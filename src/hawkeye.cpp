...
char HawkeyeCppFile[16384][256];
#define IncV(var,x) (explain ? (me ? ((var -= (x)) && fprintf(fexplain,"(%d, %d): %s [Black]: %s",Opening(-(x)),Endgame(-(x)),__FUNCTION__,HawkeyeCppFile[Min(__LINE__-1,cpp_length)])) : ((var += (x)) && fprintf(fexplain,"(%d, %d): %s [White]: %s",Opening(x),Endgame(x),__FUNCTION__,HawkeyeCppFile[Min(__LINE__-1,cpp_length)]))) : (me ? (var -= (x)) : (var += (x))))
#else
...
FILE * frec;
#endif
#define HawkeyeCpp "/Users/michaelbyrne/Documents/Sources/Hawkeye/Hawkeye-1.03.d/src.cpp"
...
void init_variables() {
	...
	if (Local) f = fopen(HawkeyeCpp, "r");
	else if (Server) f = fopen("./Server/Hawkeye.cpp", "r");
	else f = fopen("./Client/Hawkeye.cpp", "r");
	while (!feof(f)) {
...
void init_magic() {
	...
	int i, k, index, bits, bit_list[16];//mfb
	uint64 j,u;//mfb
#ifdef TUNER
...
#ifdef EXPLAIN_EVAL
	memset(HawkeyeCppFile, 0, 16384 * 256);
	FILE * fcpp; fcpp = fopen(HawkeyeCpp, "r");
	for (cpp_length = 0; cpp_length < 16384; cpp_length++) {
		if (feof(fcpp)) break;
		fgets(mstring, 65536, fcpp);
		memcpy(HawkeyeCppFile[cpp_length], mstring, 256);
		if (!strchr(HawkeyeCppFile[cpp_length], '\n')) HawkeyeCppFile[cpp_length][255] = '\n';
		char * p; for (p = HawkeyeCppFile[cpp_length]; (*p) == '\t'; p++);
		strcpy(mstring, p); strcpy(HawkeyeCppFile[cpp_length], mstring);
	}
	cpp_length--;
	fclose(fcpp);
#endif
...
template <bool me, bool pv> int q_search(int alpha, int beta, int depth, int flags) {
...
#ifdef TB
	 if (hash_depth < 0 && popcnt(PieceAll) <= TB_LARGEST ) {
	...
	if (T(hash_move)) {
		...
		if (is_legal<me>(move = hash_move)) {
			...
			if (F(Bit(To(hash_move)) & Current->mask) && F(hash_move & 0xE000) && (depth < -2 || (depth <= -1 && Current->score + 50 < alpha)) && alpha >= beta - 1 && !pv) return alpha;//mfb??
		}
	}
skip_hash_move:
...
void send_pv(int depth, int alpha, int beta, int score) {
	...
	sint64 nps, snodes, nps_time, tbhits = 0;//mfb
	...
	//nps = get_time() - StartTime;
	 nps_time = get_time() - StartTime;//mfb
#ifdef MP_NPS
	snodes = Smpi->nodes;
	tbhits = Smpi->tb_hits;
#else
	snodes = nodes;
#endif
	if (nps_time) nps = (snodes * 1000)/nps_time;
	if (score < beta) {
		if (score <= alpha) fprintf(stdout,"info time %d depth %d seldepth %d score %s%d nps %lld upperbound nodes %lld tbhits %lld pv %s\n",nps_time,depth,sel_depth,score_string,(mate ? mate_score : score),nps,snodes,tbhits,pv_string);
		else fprintf(stdout,"info time %d depth %d seldepth %d score %s%d nps %lld nodes %lld tbhits %lld pv %s\n",nps_time,depth,sel_depth,score_string,(mate ? mate_score : score),nps,snodes,tbhits,pv_string);
	} else fprintf(stdout,"info time %d depth %d seldepth %d score %s%d nps %lld lowerbound nodes %lld tbhits %lld pv %s\n",nps_time,depth,sel_depth,score_string,(mate ? mate_score : score),nps,snodes, tbhits, pv_string);}
...
void send_multipv(int depth, int curr_number) {
	...
	//sint64 nps, snodes, tbhits = 0;
	sint64 nps, nps_time, snodes, tbhits = 0;//mfb
	if (F(Print)) return;
	for (j = 0; j < PVN && T(MultiPV[j]); j++) {
	    ...
	    //if (nps) nps = (snodes * 1000)/nps;
		nps_time = get_time() - StartTime;//mfb
		if (nps_time) nps = (snodes * 1000)/nps_time;//mfb
		fprintf(stdout,"info time %d multipv %d depth %d score %s%d nps %lld nodes %lld tbhits %lld pv %s\n",nps_time,j + 1,(j <= curr_number ? depth : depth - 1),score_string,score,nps,snodes,tbhits,pv_string);
		fflush(stdout);
	}
}

void send_best_move() {
	uint64 nps,nps_time,snodes;
	int ponder, end_time;
#ifdef CPU_TIMING
...
#endif
	snodes += (snodes == 0);
	end_time = get_time() - StartTime;//mfb
	if (end_time) nps = (snodes * 1000)/end_time;//mfb
	//fprintf(stdout,"info nodes %lld score cp %d\n",snodes,best_score);
	//fprintf(stdout,"info cp %d time %d nps %lld nodes %lld\n",best_score,end_time,nps,snodes);
	fprintf(stdout,"info time %d cp %d nps %lld nodes %lld pv %s\n",end_time,best_score,nps,snodes,pv_string);
	if (!best_move) return;
	Current = Data;
	evaluate();
...
#ifndef W32_BUILD
		fprintf(stdout,"option name Hash type spin min 1 max 65536 default 64\n");
#else
...
int main(int argc, char *argv[]) {
...
#else   // LINUX, MACOSX
        WinParId = 0;
        CPUs = sysconf(_SC_NPROCESSORS_ONLN)/2;       // TODO: HT?
        PrN = CPUs;
		PrN = Min(1,CPUs);
        atexit(cleanup);
...
#ifdef TB
	if (parent && (val = getenv("Hawkeye_SYZYGY_PATH")) != NULL)
    ...
    if (parent)
		fprintf(stdout,"********************************************************\n");
		fprintf(stdout,"*  _   _                  _     ____         ____   _  *\n");
		fprintf(stdout,"* | | | |                | | _ | ___|/\\   /\\| ___| | | *\n");
		fprintf(stdout,"* | |_| | __ _ /\\  /\\  /\\| |/ /| |__ \\ \\_/ /| |__  | | *\n");
		fprintf(stdout,"* |  _  |/ _` |\\ \\/  \\/ /| / / |  __| \\   / |  __| |_| *\n");
		fprintf(stdout,"* | | | | (_| | \\  /\\  / |   \\ | |__   \\ /  | |__   _  *\n");
		fprintf(stdout,"* |_| |_|\\__,_|  \\/  \\/  |_|\\_\\|____|  |_|  |____| |_| *\n");
		fprintf(stdout,"*                                                      *\n");
		fprintf(stdout,"********************************************************\n\n");
